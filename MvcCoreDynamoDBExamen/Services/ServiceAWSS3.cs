...
namespace MvcCoreDynamoDBExamen.Services
{
    public class ServiceAWSS3
    {
        ...
        public async Task DeleteFileAsync(List<Fotos> fileName)
        {
            foreach (var fotos in fileName)
            {
                DeleteObjectResponse response =
                await this.awsClient.DeleteObjectAsync(this.bucketName, fotos.Titulo);
            }
        }
        ...
        public async Task<Stream> GetFileAsync(String fileName)
        {
            ...
            return response.HttpStatusCode == HttpStatusCode.NoContent ? response.ResponseStream : null;
        }
